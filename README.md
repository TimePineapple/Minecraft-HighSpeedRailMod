...
3. 将 `highspeedrail-1.0.0.jar` 放入服务器 `mods/`。
...
| `activeBlocks` | 加速规划格数，同时也是制动规划格数 | 格 |

20 TPS 时：`1 block/tick = 20 blocks/second`。默认 `1.2 blocks/tick` 相当于理论水平速度 `24 blocks/second`。

非法值会记录 warning 并替换为安全值。运行时若原版速度上限（例如实验 controller 的游戏规则值）高于 `maxSpeed`，模组不会降低原版上限。

加速度、减速度和扫描距离均不再是配置项。模组按 `(maxSpeed² - 原版上限²) / (2 × activeBlocks)` 实时计算每 tick 的速度变化量；默认三项得到 `0.04 blocks/tick²`。`activeBlocks` 同时控制加速和制动距离。
...
```text
NORMAL -> ACCELERATING -> HIGH_SPEED -> DECELERATING -> NORMAL
```

- `NORMAL`：完全使用原版速度上限。只有普通 `MinecartEntity` 接近原版最高速度时才扫描前方。
- `ACCELERATING`：当前动力铁轨与前方连续动力铁轨合计超过 `activeBlocks` 时启动，按规划速度变化量加速。
- `HIGH_SPEED`：允许并保持 `maxSpeed`。
- `DECELERATING`：前方连续动力铁轨少于 `activeBlocks` 时立即启动，在最后 `activeBlocks` 格内规划回到原版上限。
...
扫描每次只复用三个 `BlockPos.Mutable`，不建立路径 List；到达所需阈值立即停止。目标 chunk 未加载时调用 `isChunkLoaded` 后保守停止，不生成或强制加载区块。
...
Mixin 还在 `AbstractMinecartEntity#tick` 的 HEAD/TAIL 分别进行状态判定和速度幅值调整。TAIL 使用原版当 tick 已经修正后的速度方向，仅调整水平幅值并保留 Y 分量；位置移动、坡度、转弯、碰撞、乘客逻辑仍由原版执行。
...
```text
build/libs/highspeedrail-1.0.0.jar
```
